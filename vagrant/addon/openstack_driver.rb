#!/usr/bin/env ruby
#/var/lib/one/remotes/vmm/openstack/openstack_driver.rb
# -------------------------------------------------------------------------- #
...
OS_DRIVER_CONF = "#{ETC_LOCATION}/os_driver.conf"
OS_DRIVER_DEFAULT = "#{ETC_LOCATION}/os_driver.default"

# Load OpenStack credentials and environment
require 'yaml'
require 'rubygems'
require 'openstack'
require 'uri'
...
# The main class for the OpenStack driver
class OpenStackDriver
    ...
    def initialize(host, host_id=nil)
        ...
        @host_id = host_id
        #
        os_conf  = YAML::load(File.read(OS_DRIVER_CONF))
        #
        ...
        @os = OpenStack::Connection.create({
          :username => os_conf['username'],
          :api_key => os_conf['api_key'],
          :auth_method => os_conf['auth_method'],
          :auth_url => os_conf['auth_url'],
          :authtenant_name => os_conf['authtenant_name'],
          :service_type => os_conf['service_type'],
          :service_host => os_conf['service_host'],
          :service_scheme => os_conf['service_scheme']})

        #puts @os.connection.service_host
        @os.connection.service_host="172.29.236.10"
        @os.connection.service_scheme="http"
        @os
    end
    ...
    def deploy(id, host, xml_text, lcm_state, deploy_id)
      if lcm_state == "BOOT" || lcm_state == "BOOT_FAILURE"
        ...
        if !ec2_value(ec2_info, 'IMAGE')  # was AMI
    ...
    def monitor_all_vms
      ...
      totalcpu = 100000
      host_info =  "HYPERVISOR=openstack\n"
      ...
      usedmemory = 0
      begin
          @os.servers().each do |i|
            #puts i.inspect
              #next if i.status != :pending && i.status != :running

              one_id = nil #i.tags['ONE_ID']
...
private
    ...
    def get_deployment_info(host, xml_text)
        ...
        all_ec2_elements = xml.root.get_elements("//USER_TEMPLATE/OPENSTACK")

        # First, let's see if we have an OS site that matches
        ...
        if !ec2
            ...
            if all_ec2_elements.size == 1
                ...
                STDERR.puts("Cannot find OPENSTACK element in deployment file or no " \
                    "OPENSTACK site matching in the template.")
                exit(-1)
            end
        end
...
#puts @os.server(instance_id)

        fid = @os.server(instance_id).flavor['id']

        f = @os.flavor(fid)
        fname, cpu, mem, disk = f.name, f.vcpus, f.ram, f.disk

        cpu  = cpu.to_f   #/ 100
        vcpu = cpu.ceil
        mem  = mem.to_i #/ 1024 # Memory for templates expressed in MB

        str = "NAME   = \"OpenStack Instance #{instance_name}\"\n"\
              "CPU    = \"#{cpu}\"\n"\
              "VCPU   = \"#{vcpu}\"\n"\
              "MEMORY = \"#{mem}\"\n"\
              ...
              "  AMI   =\"#{image_id}\"\n"\
              "]\n"\
              "IMPORT_VM_ID    = \"#{instance_id}\"\n"\
              "SCHED_REQUIREMENTS=\"NAME=\\\"#{@host}\\\"\"\n"\
              "DESCRIPTION = \"Instance imported from OpenStack, from instance"\
              " #{instance_id}, ImageId #{image_id}\"\n"
